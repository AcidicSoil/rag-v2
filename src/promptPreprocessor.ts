// path: src/promptPreprocessor.ts
import {
  text,
  type Chat,
  type ChatMessage,
...
async function prepareRetrievalResultsContextInjection(
  ...
  const embeddingModelId = pluginConfig.get("embeddingModel");
  ...
  const retrievingStatus = ctl.createStatus({
    status: "loading",
    text: `Loading embedding model '${embeddingModelId}' for retrieval.`,
  });

  const embeddingModel = await ctl.client.embedding.model(embeddingModelId, {
  ...
  const result = await ctl.client.files.retrieve(originalUserPrompt, files, {
    embeddingModel,
    // Affinity threshold handled below via retrievalAffinityThreshold filter
  ...
  if (numRetrievals > 0) {
    ...
    result.entries.forEach((entry) => {
      const completeText = entry.content;
...
async function chooseContextInjectionStrategy(
  ...
  if (totalFilePlusPromptTokenCount > availableContextTokens) {
    const chosenStrategy: DocumentContextInjectionStrategy = "retrieval";
    ctl.debug(
  ...
  const chosenStrategy: DocumentContextInjectionStrategy =
    "inject-full-content";
  status.setState({
    status: "done",
